...
- **ניטור בוקר** → ‏GET ‏/api/status. שלוש רמות: ‏`healthy` — הכל תקין; ‏`degraded` (‏HTTP 200, ‏`warnings` לא ריק) — משתמשים לא נפגעים, לציין בדיווח בלי אזעקה (למשל: מקור חסום גיאוגרפית שממתין לרענון הרבעוני); ‏`unhealthy` (‏HTTP 503, ‏`problems` לא ריק) — כשל ליבה, לדווח למפעיל מיידית עם הפירוט.
...
## שינויי קוד (לסוכן שרץ על השרת, עם גישה ל-/opt/karnaf)

דחיפה ישירה לענפי הפריסה (`main`, `claude/project-deep-learning-g64ogo`)
**חסומה** ב-pre-push hook — הם מתפרסים אוטומטית לפרודקשן, ופריסה בלי אישור
אסורה. הזרימה המחייבת:

```bash
cd /opt/karnaf
git checkout -b agent/<שם-קצר-למשימה>
# … ביצוע השינוי + בדיקה מקומית (npx tsc --noEmit לכל הפחות) …
git add -A && git commit -m "תיאור ברור של השינוי ולמה"
git push -u origin agent/<שם-קצר-למשימה>
export GH_TOKEN=$(cut -d= -f2 /etc/karnaf/backup.env)
gh pr create --base claude/project-deep-learning-g64ogo \
  --title "כותרת קצרה" \
  --body "## מה השתנה\n…\n## למה\n…\n## איך נבדק\n…"
```

לאחר יצירת ה-PR: לשלוח למפעיל בטלגרם **סיכום מנהלים** — מה, למה, מה נבדק —
עם קישור ל-PR. ‏CI מריץ build אוטומטית על כל PR; **המיזוג של המפעיל הוא
שמפרס**. אסור לסוכן למזג בעצמו.

לחזרה לענף העבודה אחרי push: ‏`git checkout claude/project-deep-learning-g64ogo && git pull`.

## גבולות

- הטוקן מקנה גישת תפעול רחבה אבל **לא** אישורי מיזוג — פריסה לפרודקשן היא החלטת המפעיל בלבד.
