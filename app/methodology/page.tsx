  ...
  return (
    <main className="min-h-screen page-wrap py-8">
      ...
      <Section icon="calculator" title="חישוב ממוצע וחציון">
        ...
        <p><B>שכונות:</B> אותו חישוב בדיוק ברמת שכונה×שנה, עם סף נפרד וגבוה יותר של <B>{NB_MIN_DEALS}+ עסקאות</B> לתא — תא שכונתי קטן, ועסקה חריגה אחת מזיזה אותו. ממוצע העיר שמוצג לצד טבלת השכונות מחושב מאותן עסקאות-שכונה בלבד, ולא ממספר העיר, שכולל גם עסקאות שלא נרשמה להן שכונה. שם השכונה הוא כפי שדווח לרשות המסים; עסקה שדווחה בשני ערוצי האיסוף מקבלת את השכונה מהדיווח המקביל של אותה עסקה — העתקת עובדה, לא ניחוש — ועסקה שאין לה שכונה באף דיווח נשארת במספרי העיר בלבד. לכל שכונה שעברה את הסף יש עמוד משלה עם מגמת המחיר והיסטוריית העסקאות.</p>
      </Section>
      ...
      <Section icon="institution" title="מקורות משלימים (מסומנים תמיד)">
        <p>לצד המאגר העצמאי (<Icon name="source-own" size="1em" />) מוצגים מקורות חיצוניים מסומנים <Icon name="source-official" size="1em" />: <B>חציון גוב-נדלן הרשמי</B> (סדרה מקווקווה בגרפים), <B>למ"ס</B> (אוכלוסייה, היתרים, התחלות/גמר בנייה, דוחות), ו<B>מדדי מצב שוק</B> (מודעות, ימים בשוק). הם משלימים — לא מקור המחירים הראשי.</p>
      </Section>
