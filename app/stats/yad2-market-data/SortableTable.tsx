...
function pctClass(v: number | null | undefined, inverseGoodIsNegative = false): string {
  ...
  return isGood ? "text-emerald-600 font-semibold" : "text-rose-600 font-semibold";
}

function marketTypeBadge(t: string | null) {
  if (!t) return null;
  const map: Record<string, { he: string; cls: string }> = {
    sellers: { he: "שוק מוכרים", cls: "bg-rose-50 text-rose-700 border-rose-200" },
...
const COLUMNS: ColMeta[] = [
  ...
  { key: "new_properties", label: "מודעות חדשות", align: "center", className: "text-amber-700", defaultDir: "desc" },
  { key: "new_properties_yoy", label: "YoY חדשות", align: "center", className: "text-slate-400 text-[10px]", defaultDir: "desc" },
  { key: "secondhand_properties", label: "יד שנייה", align: "center", className: "text-cyan-700", defaultDir: "desc" },
  { key: "secondhand_yoy", label: "YoY יד2", align: "center", className: "text-slate-400 text-[10px]", defaultDir: "desc" },
  { key: "avg_days_on_market", label: "ימים בשוק", align: "center", className: "text-emerald-700", defaultDir: "asc" },
  { key: "days_yoy", label: "YoY ימים", align: "center", className: "text-slate-400 text-[10px]", defaultDir: "asc" },
  { key: "buyers_count", label: "קונים", align: "center", className: "text-purple-700", defaultDir: "desc" },
  { key: "buyers_yoy", label: "YoY קונים", align: "center", className: "text-slate-400 text-[10px]", defaultDir: "desc" },
  { key: "households", label: "משקי בית", align: "center", className: "text-rose-700", defaultDir: "desc" },
...
function SortArrow({ active, dir }: { active: boolean; dir: SortDir }) {
  ...
  return (
    <span className="text-cyan-600 mr-1 font-bold" aria-hidden>
      {dir === "asc" ? "↑" : "↓"}
    </span>
  );
}
            ...
            {COLUMNS.map((col, idx) => {
              ...
              return (
                <th
                  key={col.key}
                  scope="col"
                  className={`py-3 px-3 ${baseAlign} text-xs font-semibold cursor-pointer select-none hover:bg-slate-100 transition-colors ${col.className ?? ""} ${stickyCls} ${isActive ? "!bg-cyan-50" : ""}`}
        ...
        <tbody>
          {sortedRows.map((r) => (
            <tr key={r.city_name} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
              ...
              <td className="py-2.5 px-3 text-center text-amber-700 font-semibold">{fmt(r.new_properties)}</td>
              <td className={`py-2.5 px-3 text-center text-[11px] ${pctClass(r.new_properties_yoy)}`}>
                {fmtPct(r.new_properties_yoy)}
              </td>
              <td className="py-2.5 px-3 text-center text-cyan-700 font-semibold">{fmt(r.secondhand_properties)}</td>
              <td className={`py-2.5 px-3 text-center text-[11px] ${pctClass(r.secondhand_yoy)}`}>
                {fmtPct(r.secondhand_yoy)}
              </td>
              <td className="py-2.5 px-3 text-center text-emerald-700 font-semibold">{fmt(r.avg_days_on_market)}</td>
              <td className={`py-2.5 px-3 text-center text-[11px] ${pctClass(r.days_yoy, true)}`}>
                {fmtPct(r.days_yoy)}
              </td>
              <td className="py-2.5 px-3 text-center text-purple-700 font-semibold">{fmt(r.buyers_count)}</td>
              <td className={`py-2.5 px-3 text-center text-[11px] ${pctClass(r.buyers_yoy)}`}>
                {fmtPct(r.buyers_yoy)}
              </td>
              <td className="py-2.5 px-3 text-center text-rose-700 font-semibold">{fmt(r.households)}</td>
