...
import { toVisualRtl as rtl } from "@/lib/rtlVisual";

/**
 * The card WhatsApp, Facebook and X render when someone shares a city page.
 *
 * ⚠️ THE FONT IS NOT OPTIONAL. next/og ships exactly one font —
 * noto-sans-latin — which has no Hebrew glyphs at all. With Hebrew text and no
 * Hebrew face, the route 502'd in the container (it happened to return a blank
 * card locally, which is how this nearly shipped). So a 36KB Rubik subset
 * (Hebrew + digits + the few symbols this card uses, pinned to weight 700)
 * lives in assets/ and is handed to satori explicitly.
 *
...
/** Read once per process, not per card. */
let fontCache: Buffer | null = null;
function hebrewFont(): Buffer {
  if (!fontCache) {
    fontCache = fs.readFileSync(path.join(process.cwd(), "assets", "rubik-hebrew-700.ttf"));
  }
  return fontCache;
}
    ...
    (
      ...
      >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 14 }}>
          <span style={{ fontSize: 30, fontWeight: 800, letterSpacing: -0.5 }}>{rtl("קרנף אנליסט")}</span>
          <span style={{ fontSize: 34 }}>🦏</span>
        </div>

        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
          <div style={{ fontSize: 78, fontWeight: 900, lineHeight: 1.05 }}>{rtl(cityName)}</div>
          {sqm ? (
            <div style={{ display: "flex", alignItems: "baseline", gap: 16, direction: "rtl" }}>
              <span style={{ fontSize: 64, fontWeight: 900 }}>₪{fmt(sqm)}</span>
              <span style={{ fontSize: 30, opacity: 0.85 }}>{rtl("למ״ר")}</span>
              {changePct !== null && (
            ...
            <div style={{ fontSize: 34, opacity: 0.9 }}>{rtl("נתוני שוק הדיור — עסקאות, מחירים והיצע")}</div>
          )}
          {changeLabel && (
            <div style={{ fontSize: 24, opacity: 0.7 }}>{rtl(`שינוי מחיר ${changeLabel}`)}</div>
          )}
        </div>

        <div style={{ display: "flex", justifyContent: "flex-end", fontSize: 24, opacity: 0.75 }}>
          {rtl("עסקאות אמת מרשות המסים · נתוני למ״ס · analyst.karnafnadlan.com")}
        </div>
      </div>
    ),
    {
      ...size,
      fonts: [{ name: "Rubik", data: hebrewFont(), weight: 700, style: "normal" }],
    }
  );
}
