...
export const RULE_DEFS: RuleDef[] = [
  ...
  { key: "comp_radius_m", label: "רדיוס השוואה סביב הבניין", group: "השוואת עסקאות", kind: "number", default: 300, unit: "מטר",
    help: "כשהוזנה כתובת מדויקת (רחוב ומספר) והבניין ממוקם במפה — אחרי הרחוב ולפני השכונה נבדקות העסקאות בבניינים שבתוך המעגל הזה. רק בניינים שמוקמו ברמת בית נספרים; עסקה ללא מספר בית לעולם לא מקורבת. 0 מבטל את המדרגה." },

  { key: "partial_year_min_months", label: "מינימום חודשים לשימוש בשנה חלקית", group: "איכות מדגם", kind: "number", default: 4, unit: "חודשים",
    help: "השנה הנוכחית עדיין מתמלאת, אבל מחיר למ\"ר הוא שיעור ולא סכום — שבעה חודשי עסקאות מייצרים מספר מאותו סוג כמו שנים-עשר. לכן שנה חלקית משמשת כנקודת קצה להשוואה כשיש בה לפחות כך חודשים (כדי שלא תהיה עונה בודדת) וגם מספיק עסקאות לפי הסף הרגיל. ספירות עסקאות ממשיכות להשתמש בשנים מלאות בלבד — אלו כן גדלות עם הזמן." },
  ...
  { key: "neighborhood_ref_year_min_hoods", label: "מינימום שכונות לשנת הייחוס", group: "איכות מדגם", kind: "number", default: 3, unit: "שכונות",
    help: "שנת הייחוס של טבלת השכונות והמפה היא השנה האחרונה שבה לפחות כך וכך שכונות עברו את רף המדגם (עד שלוש שנים אחורה). בלי זה, בינואר עיר קטנה מציגה שתי שכונות של השנה החדשה במקום תשע של השנה שנגמרה — והמפה נפסלת." },

  // ── the neighbourhood MAP's display gate ──────────────────────────
  { key: "map_min_neighborhoods", label: "מינימום שכונות להצגת מפה", group: "איכות מדגם", kind: "number", default: 3, unit: "שכונות",
    help: "כמה שכונות צריכות לקבל גבול משורטט ומחיר כדי שמפת השכונות תוצג בכלל. פחות משלוש אינה מפה אלא כמה כתמים." },
  { key: "map_min_match_ratio", label: "שיעור התאמה מינימלי למפה", group: "איכות מדגם", kind: "number", default: 0.5,
    help: "איזה חלק מהשכונות שיש להן מחיר חייב לקבל גבול משורטט. מפה שחסרה בה רוב העיר גרועה מאין מפה: הקורא רואה חלל ריק וקורא אותו כ״אין שם עסקאות״, וזו טענה חזקה ושגויה יותר מאשר לא להראות מפה." },

  // ── deal PINS on the neighbourhood map ────────────────────────────
  { key: "deal_map_min_geocoded_ratio", label: "שיעור עסקאות ממוקמות מינימלי לשכבת הנקודות", group: "מפת עסקאות", kind: "number", default: 0.3,
    help: "איזה חלק מעסקאות השכונה (בטווח המוצג) חייב להיות ממוקם על המפה כדי שהנקודות יוצגו בכלל. מתחת לזה השכבה מוסתרת ומופיע משפט אחד — מפה שמראה רבע מהעסקאות נקראת כ״רק כאן נמכר״, וזו טענה שגויה." },
  { key: "deal_map_min_points", label: "מינימום נקודות לשכבת הנקודות", group: "מפת עסקאות", kind: "number", default: 5, unit: "עסקאות",
    help: "פחות מזה אינו פיזור אלא כמה נקודות בודדות, וכל אחת מהן מזוהה בקלות עם דירה ספציפית." },
  { key: "deal_map_max_points", label: "תקרת נקודות במפה", group: "מפת עסקאות", kind: "number", default: 3000, unit: "עסקאות",
    help: "כמה עסקאות לכל היותר נשלחות ומצוירות לשכונה אחת. מעבר לזה נשלחות החדשות ביותר והכיתוב אומר זאת. 3,000 נקודות הן כ-35KB דחוס — מעבר לזה הדפדפן מאט והמפה נעשית כתם." },
  { key: "deal_map_default_years", label: "טווח שנים ברירת מחדל לנקודות", group: "מפת עסקאות", kind: "number", default: 3, unit: "שנים",
    help: "כמה שנים אחורה מהשנה האחרונה של השכונה מוצגות כשפותחים אותה. הקורא יכול להרחיב; ברירת המחדל תואמת את טווח השינוי בטבלה." },

  // ── street and address PAGES ──────────────────────────────────────
  { key: "street_pages_public", label: "עמודי רחוב וכתובת פתוחים לכולם", group: "קרדיטים וגישה", kind: "boolean", default: true,
    help: "פתוח (ברירת המחדל): עמוד רחוב ועמוד בניין נגישים לכל מבקר, כולל גוגל. כבוי: הם דורשים את מה שעמוד העיר שלהם דורש — כמו עמודי השכונה." },
  { key: "street_page_min_deals", label: "מינימום עסקאות לעמוד רחוב במפת האתר", group: "איכות מדגם", kind: "number", default: 20, unit: "עסקאות",
    help: "רחוב עם פחות עסקאות מזה עדיין נפתח מקישור, אבל לא מוצע לגוגל — עמוד דל שמאונדקס מוריד את כל האתר." },
  { key: "address_page_min_deals", label: "מינימום עסקאות לעמוד בניין במפת האתר", group: "איכות מדגם", kind: "number", default: 6, unit: "עסקאות",
    help: "בניין עם פחות עסקאות מזה נפתח מקישור בלבד. שש עסקאות הן כבר היסטוריה — פחות מזה הוא דף עם טבלה של שתי שורות." },
  { key: "project_detect_min_deals", label: "זיהוי פרויקט קבלן: מינימום עסקאות חדשות", group: "סיווג עסקאות", kind: "number", default: 8, unit: "עסקאות",
    help: "בניין שבו לפחות כך עסקאות של דירה חדשה בחלון הזמן שלמטה מוצג כפרויקט — מחירי המכירה הראשונה הם מחירון קבלן, לא שוק יד שנייה, ועמוד הבניין אומר זאת." },
  { key: "project_detect_months", label: "זיהוי פרויקט קבלן: חלון חודשים", group: "סיווג עסקאות", kind: "number", default: 24, unit: "חודשים",
    help: "כמה חודשים מהעסקה החדשה הראשונה בבניין נספרים כ״שיווק הפרויקט״." },

  // ── neighbourhood PAGES ───────────────────────────────────────────
  { key: "neighborhood_pages_public", label: "עמודי שכונות פתוחים לכולם", group: "קרדיטים וגישה", kind: "boolean", default: true,
    help: "פתוח (ברירת המחדל): עמוד שכונה נגיש לכל מבקר, כולל גוגל, בלי חשבון ובלי קרדיט. כבוי: עמוד שכונה דורש את מה שעמוד העיר שלו דורש — מבקר אנונימי מופנה לעמוד העיר, ומשתמש רשום צריך שהעיר תהיה פתוחה לו. פתיחת עיר בקרדיט פותחת תמיד גם את כל שכונותיה." },
  ...
  { key: "referral_invitee_bonus", label: "בונוס לחבר שנרשם דרך הפניה", group: "קרדיטים וגישה", kind: "number", default: 5, unit: "קרדיטים",
    help: "זיכוי נוסף למי שנרשם דרך קישור של חבר, מעבר לבונוס ההרשמה הרגיל. זה מה שהופך הזמנה ממשהו שמבקשים למשהו ששווה לשלוח. 0 מבטל." },
  ...
  { key: "anon_free_cities", label: "ערים חינם למבקר לא רשום", group: "קרדיטים וגישה", kind: "number", default: 2, unit: "ערים",
    help: "כמה עמודי ערים מלאים ייפתחו למבקר בלי הרשמה, בלי שום מסך ביניים. הערים נזכרות לפי עוגייה ל-180 יום ועוברות לחשבון אם המבקר נרשם. 0 = חוזרים להתנהגות שבה כל עיר שאינה עיר הדמו דורשת חשבון. שים לב: מי שמנקה עוגיות מקבל מכסה חדשה — אין דרך אנונימית למנוע את זה, וזה חיכוך למבקר מזדמן ולא אכיפה." },

  // ── home page ─────────────────────────────────────────────────────
  { key: "hot_cities", label: "ערים חמות בעמוד הבית", group: "עמוד הבית", kind: "text", default: "חיפה,באר שבע,קריית אתא",
    help: "שלוש ערים מופרדות בפסיק, בדיוק כפי שהשם מופיע במאגר. עיר שאינה במאגר מדולגת בשקט. הערך המיוחד auto מחליף את הרשימה בשלוש הערים שהכי חיפשו באתר ב-30 הימים האחרונים — והכיתוב מתחת מתעדכן בהתאם, כי \"הכי מחופשות\" היא טענה שמותר להציג רק כשהיא נמדדת." },
  { key: "hot_city_third_metric", label: "הנתון השלישי בכרטיס עיר חמה", group: "עמוד הבית", kind: "text", default: "rooms4",
    help: "rooms4 = מחיר ממוצע לדירת 4 חדרים · sqm = מחיר ממוצע למ\"ר יד-2 · new = כמות עסקאות חדשות מקבלן · population = אוכלוסייה. שני הנתונים הראשונים בכרטיס (מגמת 3 שנים וכמות קונים יד-2) קבועים." },
];
