...
function main() {
  ...
  // Steps 2-4 run PER CITY. Both clustering modes key by city first (the exact
  // hash starts with it; the sweep fallback IS the city), so chunking by city
  // changes nothing semantically — it bounds memory to one city's rows instead
  // of the whole 1998+ scope, which OOM'd Node inside the 3GB container.
  ...
  let scanned = 0, clustersFlagged = 0, keptWholeClusters = 0, cappedBuildings = 0;
  let dropped = 0, droppedBuilding = 0, enriched = 0, crossChannel = 0;
  const perCity = new Map<string, number>();
  const samples: string[] = [];

  for (const cityName of cities) {
    // 2. this city's in-scope, keyable rows from BOTH channels — cross-channel
    //    twins are the point. Scope note: rows currently carrying an ANOMALY or
    //    SANITY flag are included; those flags are transient (flag-outlier-deals
    //    clears and recomputes them right after this script). Permanent
    //    exclusions — cross-channel merges and manual admin — stay out.
    const rows = selectRows.all(minYear, cityName) as Row[];
    scanned += rows.length;

    // 3. clustering. Exact tolerances hash into price|area|rooms blocks —
    //    complete AND near-linear. Non-zero tolerances fall back to the sweep
    //    over the whole city, capped so a wide admin value can't hang the run.
    const blocks = new Map<string, Row[]>();
    for (const r of rows) {
      const key = exact ? `${r.price}|${r.area}|${r.rooms ?? "?"}` : "*";
    ...
    clustersFlagged += candidates.length;
  ...
  console.log(`  scanned ${scanned.toLocaleString("en")} deals (active + transiently-flagged) across ${cities.length} cities…`);
  ...
  const pct = scanned ? (dropped / scanned * 100).toFixed(2) : "0";
  console.log(`  reset ${reset.changes.toLocaleString("en")} prior marks`);
  console.log(`  ${clustersFlagged.toLocaleString("en")} duplicate clusters · excluded ${dropped.toLocaleString("en")} extra copies (${pct}% of scanned)`);
